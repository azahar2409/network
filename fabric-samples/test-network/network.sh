...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Comp1 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-comp1.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating Comp2 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-comp2.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/comp1/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for Comp1 and Comp2"
...
# call the script to create the channel, join the peers of comp1 and comp2,
...
function networkDown() {
  # stop org3 containers also in addition to comp1 and comp2, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/comp1/msp organizations/fabric-ca/comp1/tls-cert.pem organizations/fabric-ca/comp1/ca-cert.pem organizations/fabric-ca/comp1/IssuerPublicKey organizations/fabric-ca/comp1/IssuerRevocationPublicKey organizations/fabric-ca/comp1/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/comp2/msp organizations/fabric-ca/comp2/tls-cert.pem organizations/fabric-ca/comp2/ca-cert.pem organizations/fabric-ca/comp2/IssuerPublicKey organizations/fabric-ca/comp2/IssuerRevocationPublicKey organizations/fabric-ca/comp2/fabric-ca-server.db'
